...
func (p *SlowLogParser) Fingerprint() string {
	p.event.Query += " " // need range to run off end of original query
	prevWord := ""
	f := make([]byte, len(p.event.Query))
	fi := 0
	...
	ReplaceNumbersInWords := p.opt.ReplaceNumbersInWords

	for qi, r := range p.event.Query {
		if Debug {
		...
		case isSpace(r):
			if s == unknown {
				...
				word := strings.ToLower(p.event.Query[cpFromOffset:qi])
		...
		case r == '(':
			if prevWord == "call" {
				...
				return "call " + p.event.Query[cpFromOffset:qi]
			} else if sqlState != onDupeKeyUpdate && (((s == inSpace || s == moreValuesOrUnknown) && (prevWord == "value" || prevWord == "values" || prevWord == "in")) || wordIn(p.event.Query[cpFromOffset:qi], "value", "values", "in")) {
		...
		case r == ':' && prevWord == "administrator":
			...
			return p.event.Query[0 : len(p.event.Query)-1] // original query minus the trailing space we added
		case r == '#':
			if Debug {
				fmt.Println("One-line comment begin")
			}
...
func (p *SlowLogParser) Id() string {
	id := md5.New()
	fpt := p.Fingerprint()
	io.WriteString(id, fpt)
